...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        index = 0
        while index < len(eliminationOrder):
        	variable = eliminationOrder[index]
        	index += 1
        	Factor_List, joined_Factor = joinFactorsByVariable(Factor_List, variable)
        	if  joined_Factor.unconditionedVariables() and len(joined_Factor.unconditionedVariables()) > 1:
        		eliminatedFactor = eliminate(joined_Factor, variable)
        		Factor_List += [eliminatedFactor]
        Joined_Factor = joinFactors(Factor_List)
        Normal_Factor = normalize(Joined_Factor)
