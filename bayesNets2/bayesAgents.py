...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    from inference import inferenceByVariableEliminationWithCallTracking
    result =  inferenceByVariableEliminationWithCallTracking()(bayesNet,[FOOD_HOUSE_VAR,GHOST_HOUSE_VAR],evidence,eliminationOrder)
    assignments = result.getAllPossibleAssignmentDicts()
    return max(assignments, key =  lambda x: result.getProbability(x))
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        """
        from inference import inferenceByVariableEliminationWithCallTracking
    	eliminated_gamestate =  inferenceByVariableEliminationWithCallTracking()(self.bayesNet,[FOOD_HOUSE_VAR,GHOST_HOUSE_VAR],evidence,eliminationOrder)
        left_dict = dict(evidence)
        right_dict = dict(evidence)
        left_dict[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        left_dict[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        right_dict[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        right_dict[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        leftExpectedValue = WON_GAME_REWARD * eliminated_gamestate.getProbability(left_dict)+ \
        	GHOST_COLLISION_REWARD * eliminated_gamestate.getProbability(right_dict)
        rightExpectedValue = WON_GAME_REWARD * eliminated_gamestate.getProbability(right_dict)+ \
        	GHOST_COLLISION_REWARD * eliminated_gamestate.getProbability(left_dict)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, evidence_temp in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue += prob * max(self.computeEnterValues(evidence_temp, enterEliminationOrder))
