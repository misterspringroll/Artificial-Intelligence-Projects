...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        for parent in node.get_parents():
            preOutput.append(self.outputs[parent])
        return preOutput
    ...
    def add(self, node):
        ...
        output = None
        #
        if len(node.get_parents()) is not 0:
    ...
    def backprop(self):
        ...
        for i in range (len(self.gradients) - 1):
            index = len(self.gradients) - i - 1

            currentNode = self.nodeList[index]

            ithParent = 0
            for parent in currentNode.get_parents():
                inputs = self.get_inputs(currentNode)
                g = self.gradients[currentNode]
                temp = currentNode.backward(inputs, g)
                self.gradients[parent] += temp[ithParent]
                ithParent += 1
    ...
    def step(self, step_size):
        ...
        for var in self.variables:
            varGradient = self.gradients[var]
            stepGradient = varGradient * step_size
            var.data = var.data - stepGradient
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        return [gradient, np.dot(np.ones(len(inputs[0])),gradient)]
